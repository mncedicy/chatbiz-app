...
export async function getOrCreateSession(phoneNumber) {
    ...
    try {
        ...
        // Safety fallback if database returns an unexpected empty row set
        if (!user) {
        ...
        if (user.id) {
           ...
           RETURNING *`,
            ...
            // Ephemeral fallback session if user ID is unassigned
        ...
        // Return safe dummy objects to prevent crashing downstream handlers
        return {
