...
export async function getOrCreateSession(phoneNumber) {
    ...
    try {
        ...
        // 1. Ensure user exists in core_users
        let userRes = await client.query(
            'SELECT id, primary_language FROM core_users WHERE phone_number = $1',
            [phoneNumber]
        );
        ...
        if (userRes.rowCount === 0) {
            const newUser = await client.query(
                `INSERT INTO core_users (phone_number, primary_language) 
         VALUES ($1, 'en') 
         RETURNING id, primary_language`,
                [phoneNumber]
            );
            userId = newUser.rows[0].id;
        } else {
            userId = userRes.rows[0].id;
            language = userRes.rows[0].primary_language || 'en';
        }
        ...
        if (sessionRes.rowCount === 0) {
            const newSession = await client.query(
                `INSERT INTO whatsapp_sessions (user_id, active_mode, current_step, session_metadata, updated_at)
...
export async function updateSession(sessionId, updates = {}) {
    ...
    if (metadata) {
        query += `, session_metadata = $${paramIdx++}::jsonb`;
